...
enrichment_scores = read.table("/data/clusterfs/lag/users/gokala/enigma-evol/partherit/results_tables/regional_hemi_spec_glob/HSE_7pcw_active_merged_results_FDR35.txt",header=T,sep="\t")
#read.table("/data/clusterfs/lag/users/gokala/enigma-evol/partherit/results_tables/regional_with_global/HSE_7pcw_active_merged_results_FDR35.txt",header=T,sep="\t")
...
g.mid = ggplot(for_bar2,aes(x=1,y=fct_rev(Region)))+geom_text(aes(label=Region),size=3.2)+
...
grid_plot1 = ggplot(data = for_bar2[for_bar2$Hemisphere=="left_prop",], aes(x = fct_rev(Region), y = Proportion)) +
  ...
  geom_text(data = label.df.left2,aes(x=Region, y=Proportion + 0.04), label = "*", color="#00786A") +
  scale_y_reverse(lim=c(0.25,-0.07)) + coord_flip()

grid_plot2 = ggplot(data = for_bar2[for_bar2$Hemisphere=="right_prop",], aes(x = fct_rev(Region), y = Proportion)) +xlab(NULL)+
  geom_bar(stat = "identity",fill="#ED6B06") + ggtitle("Right Hemisphere") +
  geom_errorbar(aes(ymin=Proportion-SE, ymax=Proportion+SE),
                width=0.1,
                position = position_dodge(width = 0.9)) +
  theme(axis.title.x = element_blank(),
        ...
        axis.ticks.y = element_blank(),
        panel.background=element_blank(),
        plot.margin = unit(c(1,0,1,-1), "mm")) + ylim(-0.07,0.25) +
  geom_text(data = label.df.right2,aes(x=Region, y=Proportion + 0.04), label = "*", color="#ED6B06") +
  coord_flip()
...
#pdf("/data/workspaces/lag/workspaces/lg-ukbiobank/projects/enigma_evol/enigma_evo/evolution/results/plots/partitioned_heritability/h2_prop/left-right_proph2_barplot_hemiSpec_cov.pdf",height = 5,width = 7)
grid.arrange(gg1,gg.mid,gg2,ncol=3,widths=c(4.2/11,2.6/11,4.2/11))
#dev.off()

for_bar2[for_bar2$Hemisphere=="left_prop",]
#ggsave("/data/workspaces/lag/workspaces/lg-ukbiobank/projects/enigma_evol/enigma_evo/evolution/results/plots/partitioned_heritability/h2_prop/left-right_proph2_barplot_hemiSpec_cov.pdf",height = 4,width = 7)
```
sessionInfo()