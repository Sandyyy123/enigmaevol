...
bedfile =  "/data/workspaces/lag/workspaces/lg-ukbiobank/projects/enigma_evol/enigma_evo/evolution/resources/new_annotations/beds/GRCh37/neanDepRegions_hg19.sorted.bed" #args[1] #
sumstatsList = "/data/clusterfs/lag/users/gokala/enigma-evol/data/european_lr/sumstats_txt_list.txt" # args[2] # 
clumpedSumstatsDir = "/data/clusterfs/lag/users/gokala/enigma-evol/eqtl/clumped_sumstats/european_lr" # args[3] # 
outDir = "/data/workspaces/lag/workspaces/lg-ukbiobank/projects/enigma_evol/enigma_evo/evolution/results/eqtl/european_lr/results" # args[4] # 
...
# Merge clumped files of all regions
...
if (length(unique(regionalAnnot$indexSNP[subjectHits(olap2)])) > 0) {
  ...
  # Convert these genes to hgnc_id
  mart = useMart(biomart="ENSEMBL_MART_ENSEMBL",dataset="hsapiens_gene_ensembl",host="feb2014.archive.ensembl.org")
  geneannot = getBM(attributes = c("refsnp_id","ensembl_gene_id","hgnc_symbol","gene_biotype"),filters="ensembl_gene_id",values=eqtlgenes,mart=mart)
  cat('These eQTLs impact ',length(which(geneannot=="protein_coding")),' protein-coding eGenes\n')
