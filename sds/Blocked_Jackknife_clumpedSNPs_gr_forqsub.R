...
phenoname = phenoname = paste(sapply(tmpname,function (x) {unlist(strsplit(x,"_",fixed=TRUE))[4]}),sapply(tmpname,function (x) {unlist(strsplit(x,"_",fixed=TRUE))[6]}),sapply(tmpname,function (x) {unlist(strsplit(x,"_",fixed=TRUE))[8]}),sep="_");
...
for (i in 1:nrow(allfileloc)) {
    ...
    mcols(newmergedGR) = merged[,c(1,4:17,19)]; # added 25th column (Z) here, otherwise "dat" didn't have newmergedGR$Z for ancestry regressed sumstats, and 19th column for non-ancreg sumstats (ENIGMA-replication sumstats)
