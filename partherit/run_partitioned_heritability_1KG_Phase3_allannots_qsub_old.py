			...
			if not os.path.exists(outDir+baseE3MA+".results"):
                           ...
                           if baseline == 'baseline':
                              if not os.path.exists(shellFile):
                                 echoLine = "echo \""+mainDir+"run_partitioned_heritability_1KG_Phase3_baseline_template.sh "+E3MA+" "+annot+" "+outDir+baseE3MA+" "+baseline+"\" > "+shellFile
                                 os.system(echoLine)
			      ...
			      if not os.path.exists(shellFile):
				 echoLine = "echo \""+mainDir+"run_partitioned_heritability_1KG_Phase3_baseline_plus_extra_template.sh "+E3MA+" "+annot+" "+outDir+baseE3MA+" "+baseline+"\" > "+shellFile
			         os.system(echoLine)
                                 os.system("chmod a+x "+shellFile)
                                 qsubLine = "qsub -o `pwd`/"+logFile+" -j y `pwd`/"+shellFile
                                 os.system(qsubLine)
			         ...
			         continue # we'll exit the script here so that the user can go delete the shell files
