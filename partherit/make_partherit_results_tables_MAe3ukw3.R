...
annots = list.dirs(path = "./partherit/ancreg_Phase3_results/", full.names = F, recursive = F)
for (i in 1:length(annots)){
      print(annots[i])
      files = Sys.glob(path = paste0("./partherit/ancreg_Phase3_results/",annots[i],"/*.gz.results"))
      ...
      for (j in 1:length(files)) {
        ...
        info2 = str_split(info1[[1]][5], pattern = "_")
        results$Annotation = info1[[1]][4]
        results$Analysis = if_else(grepl("hick",info2[[1]][3]), "Thickness", "Surface Area")
        results$Region = info2[[1]][2]
      ...
      write.table(partheritresults, 
                  paste0("./partherit/results_tables/",unique(partheritresults$Annotation),"_results_FDR35.txt"),
