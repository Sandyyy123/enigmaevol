...
for index, row in annots.iterrows():
        ...
        with open(ancregSumstats) as sumstats:
                ...
                for E3MA in sumstat_list:
                        ...
                        shellFile = "evo_partheritscripts/"+annot+baseE3MA+".sh"
                        logFile = "shelloutput/"+annot+baseE3MA+".out"
                        ...
                        if not os.path.exists(outDir+baseE3MA+".results"):
                           ...
                           if not os.path.exists(shellFile):
                              echoLine = "echo \""+"#!/bin/sh \n"+"#$ -N partherit \n"+"#$ -cwd \n"+"#$ -q multi15.q \n"+"#$ -S /bin/bash \n"+mainDir+"run_partitioned_heritability_1KG_Phase3_baseline_template.sh "+E3MA+" "+annot+" "+outDir+baseE3MA+"\" > "+shellFile
                              os.system(echoLine)
